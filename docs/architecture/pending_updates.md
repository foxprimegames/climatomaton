...
Here are the important architectural notes and requirements to be addressed in the respective design documents prior to adding their formal updates to the pending list. The previous errors regarding inbound network connectivity limits and file extensions have been omitted as those constraints are already securely handled by the central architecture and language specifications.
...
* **Codebase & Repository Architecture Document:** Needs to discuss the repository hosting platform. While the source may currently live on Codeberg, this section should reflect that the project may migrate to GitHub to remain on the same platform hosting the Nomicron code.
* **State Rehydrator Design Document:** Must clearly designate this component's responsibility to parse climate reports, ensuring the parsing logic successfully recognizes reports generated by both the bot itself and any configured administrative users. It must explicitly enforce the rule that historical end-of-turn reports are never processed during recovery.
