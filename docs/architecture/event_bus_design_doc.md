...
2. **Exception Capture & Recursive Guard:** If a subscriber raises an unhandled exception, the wrapper intercepts it to prevent the background broker loop from crashing. To prevent infinite recursive error loops (where the handling of an error generates a new, cascading error), the wrapper evaluates the originating event against a unified Recursion Guard:
   * **The Guard Condition:** If the originating `Event` has a `topic` of `sys.log`, OR if the originating `Event` contains a `Response` object with a `status` of `"exception"`, the system has identified a failure within the error-handling or logging pipeline itself.
   * **Fatal Path:** If the Guard Condition is triggered, the wrapper skips all event bus interactions, dumps the exception and traceback directly to `stderr`, and terminates the recovery process to halt the feedback loop.
   * **Standard Recovery Path:** If the Guard Condition is not met, the wrapper constructs a `Response` object containing the originating event's ID assigned to `correlation_id`, a `status` of `"exception"`, and the exception payload. This `Response` is wrapped in a new `Event` and published to the original publisher. Additionally, the wrapper publishes a `sys.log` event to report the failure to the Logging Manager.
